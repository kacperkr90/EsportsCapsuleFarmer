...
parser.add_argument('-b', '--browser', dest="browser", choices=['chrome', 'firefox', 'edge', 'remote'], default="chrome",
...
hasAutoLogin, isHeadless, username, password, browser, delay, multiplier, remoteWdHubUrl = config.getArgs()
...
try:
    driver = Webdriver(log=log, browser=browser, headless=isHeadless and hasAutoLogin, remoteWdHubUrl=remoteWdHubUrl).createWebdriver()
...
loginHandler = LoginHandler(log=log, driver=driver, multiplier=multiplier)
...
Match(log=log, driver=driver, multiplier=multiplier).watchForMatches(delay=delay)
